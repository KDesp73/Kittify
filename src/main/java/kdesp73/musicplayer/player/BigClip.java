 ...
 * javax.sound.sampled.Clip that is designed to
 * handle Clips of arbitrary size, limited only by
 * the amount of memory available to the app. It
 * uses the post 1.4 thread behaviour (daemon
 * thread) that will stop the sound running after
 * the main has exited.
 * <ul>
 * <li>2012-07-24 - Fixed bug in size of byte
 * array (2^16 -> (int)Math.pow(2, 16)).
 * <li>2009-09-01 - Fixed bug that had clip
 * ..clipped at the end, by calling drain()
 * (before calling stop()) on the dataline after
 * the play loop was complete. Improvement to
 * frame and microsecond position determination.
 * <li>2009-08-17 - added convenience constructor
 * that accepts a Clip. Changed the private
 * convertFrameToM..seconds methods from 'micro'
 * to 'milli' to reflect that they were dealing
 * with units of 1000/th of a second.
 * <li>2009-08-14 - got rid of flush() after the
 * sound loop, as it was cutting off tracks just
 * before the end, and was found to be not needed
 * for the fast-forward/rewind functionality it
 * was introduced to support.
 * <li>2009-08-11 - First binary release.
 * </ul>
 * N.B. Remove @Override notation and logging to
 * use in 1.3+
 *
 * @since 1.5
 * @version 2009-08-17
...
public class BigClip implements Clip, LineListener {
	...
	/**
	 * The end of a loop point. Defaults to the
	 * end of the Clip.
	 */
	private int loopPointEnd;

	/**
	 * Stores the current frame position of the
	 * clip.
	 */
	...
	/**
	 * Whether the sound is currently playing or
	 * active.
	 */
	private boolean active;
	/**
	 * Stores the last time bytes were dumped to
	 * the audio stream.
	 */
	...
	/**
	 * The parent Component for the loading
	 * progress dialog.
	 */
	...
	/**
	 * Default constructor for a BigClip. Does
	 * nothing. Information from the
	 * AudioInputStream passed in open() will be
	 * used to get an appropriate SourceDataLine.
	 */
	public BigClip() {
	}

	/**
	 * There are a number of AudioSystem methods
	 * that will return a configured Clip. This
	 * convenience constructor allows us to obtain
	 * a SourceDataLine for the BigClip that uses
	 * the same AudioFormat as the original Clip.
	 *
	 * @param clip Clip The Clip used to configure
	 * the BigClip.
	 */
	...
	/**
	 * Provides the entire audio buffer of this
	 * clip.
	 *
	 * @return audioData byte[] The bytes of the
	 * audio data that is loaded in this Clip.
	 */
	...
	/**
	 * Sets a parent component to act as owner of
	 * a "Loading track.." progress dialog. If
	 * null, there will be no progress shown.
	 */
	...
	/**
	 * Converts a frame count to a duration in
	 * milliseconds.
	 */
	...
	/**
	 * Converts a duration in milliseconds to a
	 * frame count.
	 */
	...
	/**
	 * Determines the single largest sample size
	 * of all channels of the current clip. This
	 * can be handy for determining a fraction to
	 * scal visual representations.
	 *
	 * @return Double between 0 & 1 representing
	 * the maximum signal level of any channel.
	 */
