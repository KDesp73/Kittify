...
/**
 * BasicPlayer is a threaded simple player class
 * based on JavaSound API. It has been
 * successfully tested under J2SE 1.3.x, 1.4.x and
 * 1.5.x.
 */
public class BasicPlayer implements BasicController, Runnable {
	...
	/**
	 * These variables are used to distinguish
	 * stopped, paused, playing states. We need
	 * them to control Thread.
	 */
	...
	/**
	 * Set SourceDataLine buffer size. It affects
	 * audio latency. (the delay between
	 * line.write(data) and real sound). Minimum
	 * value should be over 10000 bytes.
	 *
	 * @param size -1 means maximum buffer size
	 * available.
	 */
	...
	/**
	 * Set thread sleep time. Default is -1 (no
	 * sleep time).
	 *
	...
	/**
	 * Inits AudioInputStream and AudioFileFormat
	...
	protected void initAudioInputStreamParent() throws BasicPlayerException {
		try {
			...
			if (m_dataSource instanceof URL url) {
				initAudioInputStream(url);
			} else if (m_dataSource instanceof File file) {
				initAudioInputStream(file);
			} else if (m_dataSource instanceof InputStream inputStream) {
				initAudioInputStream(inputStream);
			}
			createLine();
			// Notify listeners with AudioFileFormat properties.
			Map properties = null;
			if (m_audioFileFormat instanceof TAudioFileFormat tAudioFileFormat) {
				// Tritonus SPI compliant audio file format.
				properties = tAudioFileFormat.properties();
			...
			if (audioFormat instanceof TAudioFormat tAudioFormat) {
				// Tritonus SPI compliant audio format.
				Map addproperties = tAudioFormat.properties();
	 ...
	 * Inits a DateLine.<br>
	 *
	 * We check if the line supports Gain and Pan
	 * controls.
	 *
	 * From the AudioInputStream, i.e. from the
	 * sound file, we fetch information about the
	 * format of the audio data. These information
	 * include the sampling frequency, the number
	 * of channels and the size of the samples.
	 * There information are needed to ask
	 * JavaSound for a suitable output line for
	 * this audio file. Furthermore, we have to
	 * give JavaSound a hint about how big the
	 * internal buffer for the line should be.
	 * Here, we say AudioSystem.NOT_SPECIFIED,
	 * signaling that we don't care about the
	 * exact size. JavaSound will use some default
	 ...
	 * Main loop.
	 *
	 * Player Status == STOPPED || SEEKING => End
	 * of Thread + Freeing Audio Ressources.<br>
	 * Player Status == PLAYING => Audio stream
	 * data sent to Audio line.<br>
	 * Player Status == PAUSED => Waiting for
	 * another status.
	 */
						...
						if (nBytesRead >= 0) {
							...
							while (it.hasNext()) {
								...
								if (m_audioInputStream instanceof PropertiesContainer) {
									...
									bpl.progress(nEncodedBytes, m_line.getMicrosecondPosition(), pcm2, empty_map);
								}
							}
						}
					} catch (IOException e) {
	...
	/**
	 * Skip bytes in the File inputstream. It will
	 * skip N frames matching to bytes, so it will
	 * never skip given bytes length exactly.
	 *
	 * @param bytes
	 * @return value>0 for File and value=0 for
	 * URL and InputStream
	 * @throws BasicPlayerException
	 */
	 ...
	 * @param position in the stream when the
	 * event occurs.
	 */
	...
	/**
	 * Sets Pan value. Line should be opened
	 * before calling this method. Linear scale :
	 * -1.0 <--> +1.0
	 */
	...
	/**
	 * Sets Gain value. Line should be opened
	 * before calling this method. Linear scale
	 * 0.0  <--> 1.0 Threshold Coef. : 1/2 to avoid
	 * saturation.
	 */
